...
/* Output pacing - software playback clock, see misc.c for the rationale.
 * OutputPacingWait() takes Context_t* as void* to keep this header standalone.
 *
 * Audio and video are paced against separate clocks (they are injected by
 * separate writer threads), so the caller has to say which stream the chunk
 * belongs to.
 */
#define PACING_STREAM_VIDEO 0
#define PACING_STREAM_AUDIO 1
#define PACING_STREAM_MAX   2
