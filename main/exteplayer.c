...
/* set when the user gave -b explicitly, so "-b 0" really means "no buffer"
 * and is not silently replaced by the automatic default below */
static int g_bufferSizeGiven = 0;
...
static int ParseParams(int argc,char* argv[], PlayFiles_t *playbackFiles, int *pAudioTrackIdx, int *subtitleTrackIdx, uint32_t *linuxDvbBufferSizeMB)
{
    ...
    int aopt = 0, bopt = 0;
    char *copt = 0, *dopt = 0;
    while ( (c = getopt(argc, argv, "G:W:H:A:V:U:Y:we3dlsrimva:n:x:u:c:h:o:p:P:t:9:0:1:4:f:b:F:S:O:")) != -1)
    {
        switch (c)
        {
        case 'G':
        ...
        case 'o':
            ...
            break;
        case 'Y':
            /* Software output pacing.
             * -1 -> auto (enabled on HiSilicon receivers)
             *  0 -> disabled, rely on the DVB driver back-pressure
             * >0 -> never inject more than this amount of ms ahead of real time
             */
            printf("Set output pacing lead to %d ms\n", atoi(optarg));
            OutputPacingSet(atoi(optarg));
            break;
        case 'p':
            SetNice(atoi(optarg));
            break;
...
int main(int argc, char* argv[])
{
    ...
    /* helps a lot when debugging box specific playback problems */
    printf("Detected STB type: %d (1=dreambox 2=vuplus 3=hisilicon 999=other)\n", (int)GetSTBType());
    ...
    if (0 != ParseParams(argc, argv, &playbackFiles, &audioTrackIdx, &subtitleTrackIdx, &linuxDvbBufferSizeMB))
    {
        ...
        printf("[-Y ms] output pacing: -1 auto (default), 0 off, >0 max ms injected ahead of real time\n");
    ...
    /* Without the RAM write buffer there is no read ahead at all: the demuxer
     * thread blocks inside write() to the decoder, so every network hiccup
     * starves the decoder and both picture and sound stutter. Enable the
     * buffer by default where it is needed and the user gave no -b:
     * on HiSilicon receivers, and whenever software pacing is active.
     */
    if (!g_bufferSizeGiven && 0 == linuxDvbBufferSizeMB &&
        (STB_HISILICON == GetSTBType() || OutputPacingLeadMs() > 0))
    {
        linuxDvbBufferSizeMB = 16 * 1024 * 1024;
        printf("Output pacing active, enabling %u MB write buffer\n", linuxDvbBufferSizeMB / (1024 * 1024));
    }
