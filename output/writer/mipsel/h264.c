...
static int writeData(void* _call)
{
    ...
    if (CodecData != NULL)
    {
        ...
        /* The PES header is opened with unbounded length, so it has to be
         * filled in before the first WriteV and must not be repeated for the
         * continuation batches below.
         */
        ...
        do
        {
            /* Every NAL unit needs two iovec entries (start code + payload).
             * The old check ran before appending them, so with an access unit
             * of 64 or more NAL units it wrote one entry past the end of iov[]
             * (stack overflow) and then dropped the rest of the frame, which
             * shows up as occasional picture corruption.
             * Flush the batch instead - the remaining NAL units are simply a
             * continuation of the same unbounded PES packet.
             */
            if (ic + 2 > IOVEC_SIZE)
            {
                int32_t partial = call->WriteV(call->fd, iov, ic);
                if (partial < 0)
                {
                    return partial;
                }
