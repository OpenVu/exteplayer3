...
ssize_t WriteWithRetry(Context_t *context, int pipefd, int fd, void *pDVBMtx, const void *buf, int size)
{
    ...
    {
        /* Try the write FIRST, wait only when the decoder input is really full.
         *
         * The fd is opened with O_NONBLOCK, so a write() that cannot be served
         * costs nothing and returns EAGAIN. Doing select() first - as it was
         * done before - makes every single chunk pay the select() timeout on
         * receivers where /dev/dvb/adapter0/{video0,audio0} are emulated on top
         * of a vendor media API (HiSilicon boxes): those devices never report
         * themselves as writable, so select() always ran into its timeout.
         * With ~70 chunks per second (video frames + audio frames) that is
         * several seconds of wall time per second of media - the writer thread
         * can never keep up with real time, the decoder starves and the audio,
         * which is queued behind the video, is the first thing to break up.
         */
        ...
        /* ret == 0 or EAGAIN: the decoder buffer is full, wait for space.
         *
         ...
         *      to for example 0,1s
         *
         * Both are used. The timeout is not optional either: on the emulated
         * devices mentioned above the fd never becomes writable, so a select()
         * without timeout would block forever and nothing would ever be
         * injected - the stream simply would not start.
         */
        FD_ZERO(&rfds);
        FD_ZERO(&wfds);
        ...
        if (retval > 0 && FD_ISSET(pipefd, &rfds))
        {
            FlushPipe(pipefd);
        ...
        {
            /* The device claims to be writable although the write above just
             * failed - back off a little so we do not spin in a hot loop.
             */
            usleep(1000);
        }
    }
