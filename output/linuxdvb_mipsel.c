...
int LinuxDvbAVSync(Context_t  *context, char *type __attribute__((unused))) {
     ...
     */
    if (audiofd != -1) {
        /* Broadcom/STi drivers do their own A/V sync and get confused when it is
         * requested explicitly, so 0 is kept there (historic behaviour).
         * The HiSilicon emulation on the other hand renders audio free running
         * unless sync is switched on - which makes the stream play too fast.
         */
        int32_t avSync = (STB_HISILICON == GetSTBType()) ? context->playback->AVSync : 0;

        getLinuxDVBMutex();

        if (ioctl(audiofd, AUDIO_SET_AV_SYNC, avSync) == -1)
        {
