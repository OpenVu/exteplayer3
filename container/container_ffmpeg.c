...
/* Set to the RAM output buffer size once playback starts. When the buffered
 * output is active the pacing is done by the buffering worker thread, so that
 * the demuxer can keep reading ahead into RAM. Only with buffering disabled do
 * we throttle here, in the ffmpeg thread.
 */
static uint32_t g_outputBufferSize = 0;
void LinuxDvbBuffSetPts(int64_t pts);

typedef int32_t (* Write_FN) (void  *, void *);

static int32_t Write(Write_FN WriteFun, void *context, void *privateData, int64_t pts, int32_t pacingStream)
{
    ...
    if (0 != g_outputBufferSize)
    {
        /* hand the timestamp over to the buffering layer, it paces the write
         * in the writer thread of the stream this chunk belongs to */
        LinuxDvbBuffSetPts(pts);
    }
    else
    {
        OutputPacingWait((Context_t *)context, pacingStream, pts);
    }
...
static void FFMPEGThread(Context_t *context)
{
    ...
    /* MUST be initialised. It is passed to LinuxDvbBuffSetStamp() on every
     * iteration, but was only ever assigned inside the "seek requested" branch.
     * Before the first seek it therefore held an indeterminate value, while
     * context->playback->stamp is NULL, so the buffering worker thread dropped
     * every single chunk (stamp mismatch) and nothing was ever injected:
     * with -b the playback stayed completely silent and black.
     */
    void *stamp = NULL;
    ...
    OutputPacingReset();
    stamp = context->playback->stamp;

    uint32_t bufferSize = 0;
    context->output->Command(context, OUTPUT_GET_BUFFER_SIZE, &bufferSize);
    g_outputBufferSize = bufferSize;
    ffmpeg_printf(10, "bufferSize [%u] pacing lead [%d ms]\n", bufferSize, OutputPacingLeadMs());
        ...
        if (!isWaitingForFinish && (ffmpegStatus = av_read_frame(avContextTab[cAVIdx], &packet)) == 0 )
        {
            ...
            if (videoTrack && (videoTrack->AVIdx == cAVIdx) && (videoTrack->Id == pid))
            {
...
#endif
                {
                    ...
                    if (Write(context->output->video->Write, context, &avOut, pts, PACING_STREAM_VIDEO) < 0)
                    {
            ...
            else if (audioTrack && (audioTrack->AVIdx == cAVIdx) && (audioTrack->Id == pid))
            {
                ...
                if (audioTrack->inject_raw_pcm == 1)
                {
                    ...
                    if (Write(context->output->audio->Write, context, &avOut, pts, PACING_STREAM_AUDIO) < 0)
                    {
                ...
                else if (audioTrack->inject_as_pcm == 1 && audioTrack->avCodecCtx)
                {
...
#endif
                    {
                        ...
                        if (!context->playback->BackWard && Write(context->output->audio->Write, context, &avOut, pts, PACING_STREAM_AUDIO) < 0)
                        {
            ...
            else if (subtitleTrack && (subtitleTrack->Id == pid))
            {
                ...
                if (duration > 0 || duration == -1)
                {
                    ...
                    /* INVALID_PTS_VALUE -> subtitles never drive the pacing clock */
                    if (Write(context->output->subtitle->Write, context, &subOut, (int64_t)INVALID_PTS_VALUE, PACING_STREAM_VIDEO) < 0)
                    {
